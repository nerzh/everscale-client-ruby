module TonClient
  ...
  class Abi
    ...
    # processing_try_index: Number<Optional> -     # Processing try index.    # Used in message processing with retries.Encoder uses the provided try index to calculate messageexpiration time.Expiration timeouts will grow with every retry.Default value is 0.
    # RESPONSE: ResultOfEncodeMessageBody
    # body: String -     # Message body BOC encoded with `base64`.
    # data_to_sign: String<Optional> -     # Optional data to sign.    # Encoded with `base64`. Presents when `message` is unsigned. Can be used for externalmessage signing. Is this case you need to sing this data andproduce signed message using `abi.attach_signature`.
    ...
    # call_set: CallSet<Optional> -     # Function call parameters.    # Must be specified in case of non-deploy message.In case of deploy message it is optional and contains parametersof the functions that will to be called upon deploy transaction.
    # signer: Signer -     # Signing parameters.
