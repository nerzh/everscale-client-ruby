...
describe TonClient::Tvm do
  ...
  it 'run_get_participant_list_sync' do
    state_init_source = {type: 'StateInit', source: nil, code: @elector_code, data: @elector_data, library: nil, tvc: nil, public_key: nil, init_params: nil}
    payload_encode_account = {state_init: state_init_source, balance: nil, last_trans_lt: nil, last_paid: nil}
    account_base64 = nil
    response = @client.abi.encode_account_sync(payload_encode_account)
    result = response['result']
    expect(result['account'].class).to be(String)
    account_base64 = result['account']

    payload = {account: account_base64, function_name: "participant_list", input: nil, execution_options: nil}
    response = @client.tvm.run_get_sync(payload)
    result = response['result']
    expect(result['output'][0][0][0]).to eq("0x0101b6d65a384b9c70deb49fd6c43ffc0f60ed22fcc3a4966f7043794a749228")
    expect(result['output'][0][0][1]).to eq("60138000000000")
  end

  it 'run_get_past_elections_sync' do
    state_init_source = {type: 'StateInit', source: nil, code: @elector_code, data: @elector_data, library: nil, tvc: nil, public_key: nil, init_params: nil}
    payload_encode_account = {state_init: state_init_source, balance: nil, last_trans_lt: nil, last_paid: nil}
    account_base64 = nil
    response = @client.abi.encode_account_sync(payload_encode_account)
    ...
    payload = {account: account_base64, function_name: "past_elections", input: nil, execution_options: nil}
    response = @client.tvm.run_get_sync(payload)
    result = response['result']
    expect(result['output'][0][0][0]).to eq("1588268660")
  end

  it 'run_get_compute_returned_stake' do
    state_init_source = {type: 'StateInit', source: nil, code: @elector_code, data: @elector_data, library: nil, tvc: nil, public_key: nil, init_params: nil}
    payload_encode_account = {state_init: state_init_source, balance: nil, last_trans_lt: nil, last_paid: nil}
    account_base64 = nil
    response = @client.abi.encode_account_sync(payload_encode_account)
    ...
    input = @elector_address.split(':').last
    payload = {account: account_base64, function_name: "compute_returned_stake", input: input, execution_options: nil}
    response = @client.tvm.run_get_sync(payload)
    result = response['result']
    expect(result['output'][0]).to eq("0")
  end

  private def run_message_sync
    ...
    params_of_encode_message = {abi: abi, address: nil, deploy_set: deploy_set, call_set: call_set, signer: signer, processing_try_index: nil}
    result_of_encode_message = abiEncodeMessage(
      name_abi: abi_name, 
      name_tvc: abi_name, 
      address: nil, 
      public_key: keys['public'], 
      secret_key: keys['secret'], 
      call_set_function_name: call_set[:function_name],
      call_set_header: call_set[:header],
      call_set_input: call_set[:input]
    )

    positive_balance = get_grams_from_giver_sync(client: @client, account_address: result_of_encode_message['address'])
    expect(positive_balance).to eq(true)
    
    response = @client.processing.process_message_sync({message_encode_params: params_of_encode_message, send_events: false})
    result = response['result']

    params_of_wait_for_collection = {collection: "accounts",filter: {"id": {"eq": result_of_encode_message['address']}}, result: "id boc", timeout: nil}
    result_of_wait_for_collection = nil
    response = @client.net.wait_for_collection_sync(params_of_wait_for_collection)
    result_of_wait_for_collection = response['result']
    ...
    subscription_encode_message = abiEncodeMessage(
      name_abi: abi_name, 
      name_tvc: nil, 
      address: result_of_encode_message['address'], 
      public_key: keys['public'], 
      secret_key: keys['secret'], 
      call_set_function_name: "subscribe",
      call_set_header: nil,
      call_set_input: subscribe_params
    )
    ...
    check_subscription_encode_message = abiEncodeMessage(
      ...
      public_key: keys['public'], 
      secret_key: keys['secret'], 
      call_set_function_name: "getSubscription",
      call_set_header: nil,
      call_set_input: check_subscribe_params
    )
    # p check_subscription_encode_message['message']
    params_of_run_tvm = {message: check_subscription_encode_message['message'], account: account, execution_options: nil, abi: abi}
    result_of_run_tvm = nil
    response = @client.tvm.run_tvm_sync(params_of_run_tvm)
    result_of_run_tvm = response['result']
  ...
  it 'run_tvm_sync' do
    run_message_sync do |client, encoded_message, abi, account|
      params_of_run_tvm = {
  ...
  it 'testRun_executor_sync' do
    run_message_sync do |client, encoded_message, abi, account|
      parsed = nil
      response = @client.boc.parse_account_sync({boc: account})
      result = response['result']
      parsed = result
      original_balance = parsed['parsed']['balance']
      ...
      response = @client.tvm.run_executor_sync(params_of_run_executor)
      result = response['result']
      result_of_run_executor = result

      response = @client.boc.parse_account_sync({boc: result_of_run_executor['account']})
      result = response['result']
      expect(result['parsed']['balance']).not_to eq(original_balance)
      ...
      response = @client.tvm.run_executor_sync(params_of_run_executor)
      result = response['result']
      check_result_of_run_executor = result
      expect(check_result_of_run_executor['transaction']['in_msg']).to eq(encoded_message['message_id'])
      expect(check_result_of_run_executor['fees']['total_account_fees']).to be > 0

      check_result_of_run_executor['account']
    end
  end

  it 'run_account_none_sync' do
    ...
    response = @client.boc.parse_account_sync({boc: result_of_run_executor['account']})
    result = response['result']
    expect(result['parsed']['id']).to eq("0:f18d106c11586689b11e946269ec1550b69654a8d5964de668149c28877fb65a")
    expect(result['parsed']['acc_type_name']).to eq("Uninit")
  end

  it 'run_account_uinit_sync' do
    abi_name = 'Hello'
    keys = generate_keys
    ...
    response = @client.boc.parse_account_sync({boc: result_of_run_executor['account']})
    result = response['result']
    expect(result['parsed']['id']).to eq(message['address'])
    expect(result['parsed']['acc_type_name']).to eq("Active")
  end
