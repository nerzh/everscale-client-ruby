...
describe TonClient::Tvm do
  ...
  it 'run_get_participant_list' do
    ...
    account_base64 = nil
    callLibraryMethodSync(@client.abi.method(:encode_account), payload_encode_account) do |response|
      expect(response.first.result['account'].class).to be(String)
      account_base64 = response.first.result['account']
    end

    payload = {account: account_base64, function_name: "participant_list", input: nil, execution_options: nil}
    callLibraryMethodSync(@client.tvm.method(:run_get), payload) do |response|
      expect(response.first.result['output'][0][0][0]).to eq("0x0101b6d65a384b9c70deb49fd6c43ffc0f60ed22fcc3a4966f7043794a749228")
      expect(response.first.result['output'][0][0][1]).to eq("60138000000000")
    end
  end

  it 'run_get_past_elections' do
    state_init_source = {type: 'StateInit', source: nil, code: @elector_code, data: @elector_data, library: nil, tvc: nil, public_key: nil, init_params: nil}
    payload_encode_account = {state_init: state_init_source, balance: nil, last_trans_lt: nil, last_paid: nil}
    account_base64 = nil
    callLibraryMethodSync(@client.abi.method(:encode_account), payload_encode_account) do |response|
    ...
    callLibraryMethodSync(@client.tvm.method(:run_get), payload) do |response|
      expect(response.first.result['output'][0][0][0]).to eq("1588268660")
    end
  end
  ...
  it 'run_get_compute_returned_stake' do
    ...
    callLibraryMethodSync(@client.tvm.method(:run_get), payload) do |response|
      expect(response.first.result['output'][0]).to eq("0")
    end
  end

  private def run_message
    ...
    callLibraryMethodSync(@client.processing.method(:process_message), {message_encode_params: params_of_encode_message, send_events: false})
    ...
    callLibraryMethodSync(@client.tvm.method(:run_tvm), params_of_run_tvm) do |resp|
      result_of_run_tvm = resp.first.result
    end
  ...
  it 'testRun_executor' do
    run_message do |client, encoded_message, abi, account|
      ...
      callLibraryMethodSync(@client.tvm.method(:run_executor), params_of_run_executor) do |response|
        result_of_run_executor = response.first.result
      end
      ...
      callLibraryMethodSync(@client.tvm.method(:run_executor), params_of_run_executor) do |response|
        check_result_of_run_executor = response.first.result
      end
  ...
  it 'run_account_none' do
    ...
    callLibraryMethodSync(@client.boc.method(:parse_account), {boc: result_of_run_executor['account']}) do |response|
      expect(response.first.result['parsed']['id']).to eq("0:f18d106c11586689b11e946269ec1550b69654a8d5964de668149c28877fb65a")
      expect(response.first.result['parsed']['acc_type_name']).to eq("Uninit")
    end
  end
  ...
  it 'run_account_uinit' do
    ...
    callLibraryMethodSync(@client.boc.method(:parse_account), {boc: result_of_run_executor['account']}) do |response|
      expect(response.first.result['parsed']['id']).to eq(message['address'])
      expect(response.first.result['parsed']['acc_type_name']).to eq("Active")
    end
  end
end
