...
describe TonClient::TonBinding do
  ...
  it 'async requestLibrary' do
    ...
    expect(response.result).to eq({"version"=>"1.0.0"})
...
describe TonClient::Crypto do
  ...
  it 'random_generate_bytes' do
    payload = {length: 32}
    callLibraryMethodSync(@client.crypto.method(:random_generate_bytes), payload) do |response|
...
describe TonClient::Abi do
  ...
  it 'encode_message' do
    ...
    callLibraryMethodSync(@client.crypto.method(:encode_message), payload) do |response|
      expect(response.first.result['public']).to eq('02a2b8a753c8e6c3d057b6956d125935288e657eee5ae0950095b2660d0159727f')
    end
  end
end
